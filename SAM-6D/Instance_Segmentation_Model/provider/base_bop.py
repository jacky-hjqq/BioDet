...
class BaseBOP(Dataset):
    ...
    def load_metaData(self, reset_metaData, mode="query", split="test", level=2):
        ...
        if mode == "query":
            ...
            if dataset_name in ["xyzibd", "ipd"]:
                target_path = osp.join(self.root_dir, f"test_targets_bop24.json")
                targets = load_json(target_path)
                scene_to_ims = defaultdict(list)
                for t in targets:
                    scene_to_ims[t["scene_id"]].append(t["im_id"])
            ...
            if reset_metaData:
                for scene_path in tqdm(self.list_scenes, desc="Loading metaData"):
                    ...
                    if osp.exists(osp.join(scene_path, "rgb")):
                        ...
                        if dataset_name in ['itodd']:
                            rgb_paths = sorted(Path(scene_path).glob("gray/*.tif"))
                            depth_paths = sorted(Path(scene_path).glob("depth/*.tif"))
                        elif dataset_name in ['ipd', 'xyzibd']:
                            scene_id_num = int(scene_id.split("_")[-1])
                            image_ids = scene_to_ims[scene_id_num] 
                            scene_dir = Path(scene_path)
                            rgb_paths   = [scene_dir / "gray"  / f"{image_id:06d}.png" for image_id in image_ids]
                            depth_paths = [scene_dir / "depth" / f"{image_id:06d}.png" for image_id in image_ids]
