...
class BOPTemplate(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for index, id_template in enumerate(self.index_templates):
            image = Image.open(
                f"{self.template_dir}/obj_{self.obj_ids[idx]:06d}/rgb_{index}.png"
            )
            boxes.append(image.getbbox())

            # mask = image.getchannel("A")
            mask = Image.open(
                f"{self.template_dir}/obj_{self.obj_ids[idx]:06d}/mask_{index}.png"
            )
...
class BaseBOPTest(BaseBOP):
    ...
    def load_depth_img(self, idx):
        ...
        if depth_path is None:
            if self.root_dir.split("/")[-1] in ["itodd", "xyzibd", "ipd"]:
