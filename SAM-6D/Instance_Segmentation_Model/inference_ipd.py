...
if __name__ == "__main__":
    ...
    for scene_id in scene_ids:
        ...
        rgb_dir = os.path.join(scene_dir, "rgb_photoneo")
        depth_dir = os.path.join(scene_dir, "depth_photoneo")
        camera_json_path = os.path.join(scene_dir, "scene_camera_photoneo.json")
        with open(camera_json_path, "r") as f:
            camera_info = json.load(f)

        im_ids = sorted(os.listdir(depth_dir))
        for im_id, img in tqdm(enumerate(im_ids)):
            # check if the image is in the targets
            if not any(item["im_id"] == int(os.path.splitext(img)[0]) and item["scene_id"] == scene_id for item in targets):
                continue
            ...
            if infer_dataset_cfg["use_depth"]:
                # Make a mask of valid pixels
                valid_mask = (depth >= depth_range[0]) & (depth <= depth_range[1])
                d = np.zeros_like(depth)
                d[valid_mask] = (depth[valid_mask] - depth[valid_mask].min()) / (depth[valid_mask].max() - depth[valid_mask].min())
                depth_image = mpl.colormaps['plasma'](d)*255
                depth_image = depth_image.astype(np.uint8)[:,:,:-1]
                res = crop_frame(gdino, Mobile_SAM, depth_image, scale=1.0)
            else:
                # Use the rgb image for cropping
                ...
                # # Save the image
                # save_path = os.path.join(output_dir, f"{im_id:06d}.png")
                # bbox_annotated_pil.save(save_path)
                # continue

                # Crop the image based on the bounding box
                # cropped_rgb, cropped_depth, mask_frame, bbox = crop_image(res, enhanced_rgb, depth)
                cropped_rgb, cropped_depth, mask_frame, bbox = crop_image(res, rgb, depth)  
            ...
            batch = {}
            # rgb to (0,1)
            cropped_rgb = cropped_rgb.astype(np.float32) / 255.0

            batch['image'] = torch.from_numpy(cropped_rgb).permute(2, 0, 1).unsqueeze(0).float().cuda()
