...
try:
    ...
    # Get the current working directory if in a notebook
    BASE_DIR = os.getcwd()
...
class ObjectPredictor(Logger):
    ...
    def bbox_to_scaled_xyxy(self, bboxes: torch.tensor, img_w, img_h):
        ...
        """
        try:
            bboxes = bboxes * torch.Tensor([img_w, img_h, img_w, img_h]).cpu()
...
class GroundingDINOObjectPredictor(ObjectPredictor):
    ...
    """
    def __init__(self, config_file, ckpt_path, threshold=0.10):
        super().__init__()
...
class SegmentAnythingPredictor(ObjectPredictor):
    ...
    def __init__(self, vit_model="vit_t"):
        ...
        super().__init__()
        sam_weight_path = {
            "vit_t": "SAM-6D/Instance_Segmentation_Model/checkpoints/mobilesam/vit_t.pth",
